...
Команда ```git add``` не сохраняет файлы, а лишь "помечает" их для следующего сохранения (коммита). Это как положить товары в корзину перед оплатой в интернет-магазине.
...
### Перенос (клонирование) проекта с GitHub на ноутбук
Этот шаг нужен, если проект уже существует на GitHub, и вы хотите начать с ним работать на своем компьютере.

1. Скопируйте URL репозитория на GitHub:
2. Зайдите на страницу вашего репозитория на GitHub.
3. Нажмите зеленую кнопку <> Code и скопируйте URL (например, https://github.com/ваш-логин/ваш-репозиторий.git).
4. Откройте терминал в VS Code: View (Вид) -> Terminal (Терминал) или горячие клавиши Ctrl + (Backtick).
5. Введите команду ```git clone``` и вставьте ссылку
**Что делает:** Скачивает всю папку проекта (включая служебную папку .git с историей версий) с GitHub в текущую директорию на вашем ноутбуке.
**Результат:** У вас появляется локальная копия проекта. Папка будет называться так же, как и ваш репозиторий на GitHub.
Git автоматически запомнил адрес удаленного репозитория под именем origin. Чтобы перейти в репозиторий используется команда ```cd```

### Cохранение версии в локальном репозитории
1. Сделайте комит: ```git commit -m "Краткое описание изменений"```
**Что делает:** Сохраняет текущее состояние всех файлов, которые вы добавили через git add, в вашу локальную базу данных Git.
2. Отправьте коммиты на GitHub: ```git push origin main```
